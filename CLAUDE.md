...
Textual TUI, task queue, cron scheduler, email channel, web search, measurement rig. **1,135 offline tests**, green with no API key, no network, a
...
| real repositories | **7/12** at 2 runs per case, 2026-09-12, against **10/18** at 3 runs on 2026-09-03 - FLAT across ~17 commits, none aimed here. Three cases 1/2 and unreadable; `cachetools` (was 3/3) is the one to re-run. Median tokens +45%, 242k -> 351k; 2 of 7 passes ended `stuck`/`budget` with the goal already met. Earlier: **10/18**, all six cases x 3 runs. A 4.6x larger model (`nemotron-3-ultra-550b-a55b`) scored **10/18 too** - four cases moved, the total did not, and it spent 9% FEWER tokens. `real-humanize` 0/3 -> **2/3** on ultra, the first movement in 13 runs, and worth repeating |
...
pytest                            # 1,135 tests, no API key, no network
```
